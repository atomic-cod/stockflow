...
export default async function Dashboard() {
  ...
  const { data: products } = await supabase.from("products").select("id,name,stock_quantity,minimum_stock,cost_price,sale_price").eq("active", true);
  ...
  const { data: monthMovements } = await supabase.from("stock_movements").select("type,quantity").gte("created_at", monthStart);
  ...
  return <div className="shell"><Sidebar/><main className="main">
    ...
    <section className="grid section"><div className="card"><span className="muted">Entradas no mês</span><div className="stat">{entries.toLocaleString("pt-BR")}</div></div><div className="card"><span className="muted">Saídas no mês</span><div className="stat">{exits.toLocaleString("pt-BR")}</div></div><Link href="/produtos" className="card"><b>Produtos</b><p className="muted">Catálogo, preços e estoque.</p></Link><Link href="/relatorios" className="card"><b>Relatórios</b><p className="muted">Movimentações e indicadores.</p></Link></section>
    <section className="section card"><h2>Últimas movimentações</h2><div className="table-wrap"><table className="table"><thead><tr><th>Produto</th><th>Tipo</th><th>Quantidade</th><th>Estoque após</th><th>Data</th></tr></thead><tbody>{movements?.map(m=><tr key={m.id}><td>{Array.isArray(m.products)?m.products[0]?.name:m.products?.name}</td><td>{m.type}</td><td>{m.quantity}</td><td>{m.new_quantity}</td><td>{new Date(m.created_at).toLocaleString("pt-BR")}</td></tr>)}{!movements?.length&&<tr><td colSpan={5}>Nenhuma movimentação registrada.</td></tr>}</tbody></table></div></section>
  </main></div>;
}