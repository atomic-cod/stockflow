...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const patch={name:String(body.name??"").trim(),sku:String(body.sku??"").trim(),barcode:String(body.barcode??"").trim()||null,description:String(body.description??"").trim()||null,cost_price:Number(body.cost_price??0),sale_price:Number(body.sale_price??0),minimum_stock:Number(body.minimum_stock??0),maximum_stock:body.maximum_stock===""||body.maximum_stock==null?null:Number(body.maximum_stock),location:String(body.location??"").trim()||null};
