...
export default async function Relatorios() {
  ...
  return <div className="shell"><Sidebar/><main className="main"><div className="topbar"><div><h1 className="title">Relatórios</h1><p className="muted">Indicadores operacionais do estoque.</p></div></div>
    ...
    <section className="section card"><h2>Histórico</h2><div className="table-wrap"><table className="table"><thead><tr><th>Data</th><th>Produto</th><th>Tipo</th><th>Qtd.</th><th>Anterior</th><th>Novo</th><th>Motivo</th></tr></thead><tbody>{movements?.map(m=><tr key={m.id}><td>{new Date(m.created_at).toLocaleString("pt-BR")}</td><td>{Array.isArray(m.products)?m.products[0]?.name:m.products?.name}</td><td>{m.type}</td><td>{m.quantity}</td><td>{m.previous_quantity}</td><td>{m.new_quantity}</td><td>{m.reason||"—"}</td></tr>)}{!movements?.length&&<tr><td colSpan={7}>Nenhuma movimentação.</td></tr>}</tbody></table></div></section>
  </main></div>;
}